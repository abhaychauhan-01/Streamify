...
export async function getMyFriends(req, res) {
  try {
    // populate is used to get friends data , if we only want friends it then we will not use it
  ...
  try {
    ...
    if (existingRequest)
      return res.status(400)({
        message: "A friend Request exists between you and this user",
      });
...
export async function acceptFriendRequest(req, res) {
  try {
    const { id: requestId } = req.params; // destructuring req. params and getting Id from it and renaming it as requestId
    ...
    await User.findByIdAndUpdate(friendRequest.sender, {//updateing sender's friends array/list
      $addToSet: { friends: friendRequest.recipient },
    });
    await User.findByIdAndUpdate(friendRequest.recipient, {//updateing recipient's friends array/list
...
export async function getFriendRequests(req,res){
    try {const incomingReqs=await FriendRequest.find({
